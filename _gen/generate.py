...
COOKIE_BANNER = '''<div id="cookie-banner" style="transform:translateY(120%);transition:transform .25s ease;will-change:transform;position:fixed;left:0;right:0;bottom:0;z-index:60;background:#ffffff;border-top:1px solid #222222;padding:14px 18px;font-size:13.5px;color:#222222;line-height:1.5">
  <div style="max-width:1080px;margin:0 auto;display:flex;flex-wrap:wrap;gap:12px;align-items:center;justify-content:space-between">
    <span>We use cookies for analytics and advertising (incl. Google AdSense). See our <a href="/cookies.html" style="color:#222222">Cookie Policy</a>.</span>
    <span><button onclick="cookieOK(0)" style="background:none;border:1px solid #d5d9e0;color:#384250;padding:7px 14px;border-radius:9px;cursor:pointer;margin-right:8px">Reject</button><button onclick="cookieOK(1)" style="background:#222222;border:0;color:#fff;padding:7px 16px;border-radius:8px;cursor:pointer;font-weight:500">Accept</button></span>
  </div>
</div>
...
<link rel="apple-touch-icon" href="/apple-touch-icon.png"><link rel="manifest" href="/manifest.json"><meta name="theme-color" content="#222222">
...
<div id="cookie-banner" style="transform:translateY(120%);transition:transform .25s ease;will-change:transform;position:fixed;left:0;right:0;bottom:0;z-index:60;background:#222222;border-top:1px solid #222222;padding:14px 18px;font-size:13.5px;color:#ffffff;line-height:1.5">
  <div style="max-width:1080px;margin:0 auto;display:flex;flex-wrap:wrap;gap:12px;align-items:center;justify-content:space-between">
    ...
    <span><button onclick="cookieOK(0)" style="background:none;border:1px solid rgba(255,255,255,.4);color:#ffffff;padding:7px 14px;border-radius:8px;cursor:pointer;margin-right:8px">Reject</button><button onclick="cookieOK(1)" style="background:#ffffff;border:0;color:#222222;padding:7px 16px;border-radius:8px;cursor:pointer;font-weight:500">Accept</button></span>
  </div>
</div>
<script>(function(){try{if(!localStorage.getItem('cookie_consent'))document.getElementById('cookie-banner').style.transform='none';}catch(e){}})();
function cookieOK(v){try{localStorage.setItem('cookie_consent',v?'accepted':'rejected');}catch(e){}if(window.__grantConsent)__grantConsent(!!v);document.getElementById('cookie-banner').style.transform='translateY(120%)';}</script>
</body>
</html>
"""
...
<div class="psearch" style="max-width:540px;margin:0 auto 26px;position:relative">
  <input id="q" type="search" placeholder="Search articles…" aria-label="Search articles" autocomplete="off"
    style="width:100%;box-sizing:border-box;padding:13px 20px 13px 46px;border-radius:8px;border:1px solid #ebebeb;background:#ffffff;font:inherit;font-size:15px;outline:none">
    ...
    if lp.exists():
        ...
        if TA in lc and TB in lc:
            ...
            for label, appkey, svg in cats:
                ...
                lis = "".join(
                    f'<li><a href="/blog/{pp["slug"]}/" title="{html.escape(pp["title"])}">{html.escape(pp["title"].split(":")[0].strip())}</a></li>'
                    for pp in picks)
                cards += (f'<div style="opacity:1;transform:none"><h3 style="font-family:\'Sora\',sans-serif;font-size:18px;font-weight:500;margin-bottom:6px;display:flex;align-items:center;gap:8px">'
                          ...
                          f'<ul class="topiclist">{lis}</ul></div>')
            n_total = len(posts)
